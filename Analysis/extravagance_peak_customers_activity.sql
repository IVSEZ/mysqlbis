...
select * from rcbill_my.customercontractactivity WHERE CLIENTCODE='I8985'; limit 100;
...
/*
select clientcode, clientname, clientclass, network, contractcode, activecount, region from rcbill_my.customercontractactivity where period in 
(
...
select periodmth, periodyear, clientcode, package, count(*) as activecount 
from
...
and package in ('Extravagance','Extravagance Corporate')
and REPORTED='Y'
group by periodmth, periodyear, clientcode, package
order by periodmth, periodyear
;


select clientcode, package,
...
drop table if exists rcbill_my.rep_extravagance_activity;
...
create table rcbill_my.rep_extravagance_activity
as
(
	...
	(
		...
		select clientcode as client_code, package,
		ifnull(sum(`201601`),0) as `201601`,
		ifnull(sum(`201602`),0) as `201602`,
		ifnull(sum(`201603`),0) as `201603`,
		ifnull(sum(`201604`),0) as `201604`,
		...
		*/
		...
		(
			...
			*/
			...
			from 
			(
				select periodmth, periodyear, clientcode, package, count(*) as activecount, sum(ACTIVECOUNT) as activecount2 
				from
				rcbill_my.customercontractactivity
				where
				clientcode in 
