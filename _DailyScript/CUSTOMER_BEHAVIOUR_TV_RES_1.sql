...
set @datee='2018-09-30';
    ...
    create table rcbill_my.tempcustbehr1 as 
    (
		SELECT 
			period,
				periodday,
    ...
    create table rcbill_my.tempcustbehr2 as 
    (
    ...
    select * from rcbill_my.tempcustbehr1;
    
    select * from rcbill_my.tempcustbehr2 
    ;
    -- where standalone>1;
...
####################################################
### PIVOT
select 'RES' as ctype, concat(standalone,"|", addon) as combo, standalone, addon, periodyear, periodmth
, count(distinct clientcode) as dcl
from 
rcbill_my.tempcustbehr2 
group by 1, 2, standalone, addon, periodyear, periodmth
order by standalone, addon, periodyear, periodmth
; 


select 'RES' as ctype, periodyear, periodmth
, count(distinct clientcode) as dcl
from 
rcbill_my.tempcustbehr2 
...
set @datee='2018-03-31';
    select * from rcbill_my.tempcustbehr2 
    where (period>=@dates and period<=@datee)
    ;
 
 
set @dates='2018-04-01';
set @datee='2018-06-30';
    select * from rcbill_my.tempcustbehr2 
    where (period>=@dates and period<=@datee)
    ;

set @dates='2018-07-01';
set @datee='2018-09-30';
    select * from rcbill_my.tempcustbehr2 
    where (period>=@dates and period<=@datee)
    ;
