...
LOAD DATA LOW_PRIORITY LOCAL INFILE 'C:\\ProgramData\\MySQL\\MySQL Server 5.7\\Uploads\\rcbill\\AllTClients-15012023.csv' 
...
select count(*) as rcb_clientphones from rcbill.rcb_clientphones;
...
, (ifnull(b.areaname,'TBU')) as ClientArea
, (ifnull(b.districtname,'TBU')) as ClientSubDistrict
...
LOAD DATA LOW_PRIORITY LOCAL INFILE 'C:\\ProgramData\\MySQL\\MySQL Server 5.7\\Uploads\\rcbill\\AllContracts-15012023.csv'
...
LOAD DATA LOW_PRIORITY LOCAL INFILE 'C:\\ProgramData\\MySQL\\MySQL Server 5.7\\Uploads\\rcbill\\AllContractServices-15012023.csv' 
...
LOAD DATA LOW_PRIORITY LOCAL INFILE 'C:\\ProgramData\\MySQL\\MySQL Server 5.7\\Uploads\\rcbill\\AllContractDiscounts-15012023.csv'
...
CREATE INDEX IDXCCD2
ON rcbill.clientcontractdiscounts (clientid);
CREATE INDEX IDXCCD3
ON rcbill.clientcontractdiscounts (contractid);
...
LOAD DATA LOW_PRIORITY LOCAL INFILE 'C:\\ProgramData\\MySQL\\MySQL Server 5.7\\Uploads\\rcbill\\AllDevices-15012023.csv' 
...
;
	...
	CREATE INDEX idxccd7
	ON rcbill.clientcontractdevices (contractid, csid);

select count(*) as devices from rcbill.rcb_devices;
select count(*) as clientcontractdevices from rcbill.clientcontractdevices;
-- select * from rcbill.clientcontractdevices where clientcode='I.000011750' limit 100
...
(
	...
	select a.*, upper(trim(b.VENDORNAME)) as MAC_VENDOR
    from 
    (
		select a.*, upper(trim(replace(left(a.mac,8),'.',':'))) as mac_prefix
        from 
        (
			select 
			...
			, case when length(ccd.phoneno)=17 then replace(ccd.phoneno,'.',':') else ccd.phoneno end as UID
			, case when length(ccd.mac)=17 then replace(ccd.mac,'.',':') else ccd.mac end as mac
			, case when length(ccd.SerNo)=17 then replace(ccd.SerNo,'.',':') else ccd.SerNo end as SerNo
...
LOAD DATA LOW_PRIORITY LOCAL INFILE 'C:\\ProgramData\\MySQL\\MySQL Server 5.7\\Uploads\\rcbill\\AllTickets-15012023.csv' 
...
CREATE INDEX IDXtick1 ON rcb_tickets (CLID,CID);
...
ON rcb_tickets (OPEN_D);

/*
CREATE INDEX IDXtick5
ON rcb_tickets (CID);

drop index IDXtick1 ON rcbill.rcb_tickets;
*/

select count(*) as tickets from rcb_tickets;
-- select * from rcb_tickets;
...
LOAD DATA LOW_PRIORITY LOCAL INFILE 'C:\\ProgramData\\MySQL\\MySQL Server 5.7\\Uploads\\rcbill\\AllTicketTechUsers-15012023.csv' 
...
-- select * from rcb_tickettechusers;
...
LOAD DATA LOW_PRIORITY LOCAL INFILE 'C:\\ProgramData\\MySQL\\MySQL Server 5.7\\Uploads\\rcbill\\AllTicketAssignments-15012023.csv' 
-- REPLACE INTO TABLE `rcbill`.`rcb_tickets` CHARACTER SET UTF8 FIELDS TERMINATED BY ',' 
...
LOAD DATA LOW_PRIORITY LOCAL INFILE 'C:\\ProgramData\\MySQL\\MySQL Server 5.7\\Uploads\\rcbill\\AllTicketComments-15012023.csv' 
...
LOAD DATA LOW_PRIORITY LOCAL INFILE 'C:\\ProgramData\\MySQL\\MySQL Server 5.7\\Uploads\\rcbill\\AllComments-15012023.csv' 
...
LOAD DATA LOW_PRIORITY LOCAL INFILE 'C:\\ProgramData\\MySQL\\MySQL Server 5.7\\Uploads\\rcbill\\AllDevicesOld-15012023.csv' 
...
LOAD DATA LOW_PRIORITY LOCAL INFILE 'C:\\ProgramData\\MySQL\\MySQL Server 5.7\\Uploads\\rcbill\\AllInvoicesContents-15012023.csv' 
...
CREATE INDEX IDXinvoicescontents5
...
LOAD DATA LOW_PRIORITY LOCAL INFILE 'C:\\ProgramData\\MySQL\\MySQL Server 5.7\\Uploads\\rcbill\\AllInvoicesHeader-15012023.csv' 
...
Rate=@Rate ,
ID_OLD=@ID_OLD ,
ORDERID=trim(replace(@ID_OLD,"wfl.",""))*1,
UPDDATE=@UPDDATE ,
USERID=@USERID ,
DueDate=@DueDate ,
InvNo=@InvNo ,
...
CREATE INDEX IDXinvoiceheader6
ON rcb_invoicesheader (PaymentID);
...
LOAD DATA LOW_PRIORITY LOCAL INFILE 'C:\\ProgramData\\MySQL\\MySQL Server 5.7\\Uploads\\rcbill\\AllUsers-15012023.csv' 
...
-- SET SESSION sql_mode = '';
LOAD DATA LOW_PRIORITY LOCAL INFILE 'C:\\ProgramData\\MySQL\\MySQL Server 5.7\\Uploads\\rcbill\\AllUserActions2019-15012023.csv' 
...
LOAD DATA LOW_PRIORITY LOCAL INFILE 'C:\\ProgramData\\MySQL\\MySQL Server 5.7\\Uploads\\rcbill\\AllIPUSAGE-15012023.csv' 
...
;
	...
	ON rcbill.rcb_ipusage1 (cid, csid);
...
LOAD DATA LOW_PRIORITY LOCAL INFILE 'C:\\ProgramData\\MySQL\\MySQL Server 5.7\\Uploads\\rcbill\\AllCASA-15012023.csv' 
...
select 'Updated CASA' as message;
...
## FIRST TIME
## 3177583 row(s) affected Records: 3177583  Duplicates: 0  Warnings: 0 437.125 sec

/*
drop table if exists rcbill.rcb_paid_subscriptions;

create table rcbill.rcb_paid_subscriptions(index idxrps1(CASA_ID), index idxrps2(CLIENT_ID, CONTRACT_ID), index idxrps3(ClientCode, ContractCode))
as
(
...
CREATE INDEX idxrps4 ON rcbill.rcb_paid_subscriptions(CLIENT_ID);
CREATE INDEX idxrps5 ON rcbill.rcb_paid_subscriptions(CONTRACT_ID);
CREATE INDEX idxrps6 ON rcbill.rcb_paid_subscriptions(ClientCode);
CREATE INDEX idxrps7 ON rcbill.rcb_paid_subscriptions(ContractCode);

*/


### EVERY OTHER TIME
insert into rcbill.rcb_paid_subscriptions
(
...
select count(*) as rcb_paid_subscriptions from rcbill.rcb_paid_subscriptions;
...
select count(*) as clientpayments from rcbill.clientpayments;
*/
...
######################################################################
drop table if exists rcbill.clientcontractsservicepackageprice;

create table rcbill.clientcontractsservicepackageprice as 
(
	select 
    cl.kod as clientcode
    , upper(cl.firm) as clientname
    , a.kod as contractcode -- , b.servicerateid
    , ifnull(a.currency,'SCR') as currency
	, ifnull((select name from rcbill.rcb_ratingplans where ID=a.ratingplanid),'MAHE_SU_STANDARD_20110601') as RatingPlanName    
	, (select `name` from rcbill.rcb_creditpolicy where ID=a.CreditPolicyID) as CreditPolicyName
	, (select MaxInvDate from rcbill.rcb_creditpolicy where ID=a.CreditPolicyID) as MaxInvDate
	, (select MinInvDate from rcbill.rcb_creditpolicy where ID=a.CreditPolicyID) as MinInvDate
    ...
    , a.InvoicingDate as InvoicingDate
    -- , if(a.enddate is not null,0,a.active) as contractstatus
    , if((a.enddate is not null and a.enddate<=now()),0,a.active) as contractstatus
    
    
    -- , ifnull(a.enddate,1) as contractstatus
...
create index IDXccspp1 on rcbill.clientcontractsservicepackageprice (clientcode);
create index IDXccspp2 on rcbill.clientcontractsservicepackageprice (contractcode);
create index IDXccspp3 on rcbill.clientcontractsservicepackageprice (servicerateid);
create index IDXccspp4 on rcbill.clientcontractsservicepackageprice (service);
-- create index IDXcspp2 on rcbill.contractsservicepackageprice (package);
-- create index IDXcspp5 on rcbill.contractsservicepackageprice (USAGE_YR);
...
/*
drop table if exists rcbill.clientcontractsservicepackagepricedevice;

create table rcbill.clientcontractsservicepackagepricedevice as 
(
			select a.*, b.CONTRACT_TYPE,b.SERVICE_TYPE,b.FSAN,b.MAC,b.UID,b.username
...
create index IDXccsppd1 on rcbill.clientcontractsservicepackagepricedevice (clientcode);
create index IDXccsppd2 on rcbill.clientcontractsservicepackagepricedevice (contractcode);

select count(*) as clientcontractsservicepackagepricedevice from rcbill.clientcontractsservicepackagepricedevice;
*/