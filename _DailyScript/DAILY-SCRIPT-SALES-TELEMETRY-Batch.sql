...
-- LOAD DATA LOW_PRIORITY LOCAL INFILE 'C:\\ProgramData\\MySQL\\MySQL Server 5.7\\Uploads\\_csv\\SalesReport-25082019-02092019-1.csv' 
 LOAD DATA LOW_PRIORITY LOCAL INFILE 'C:\\ProgramData\\MySQL\\MySQL Server 5.7\\Uploads\\_csv\\SalesReport2-18112022-20112022-1.csv' 
...
select date(orderdate) as orderdate, count(*) as records from rcbill_my.dailysales group by 1 order by 1 desc limit 15;
...
Completed::10
Canceled::1
Canceled Officialy::101


ORDERTYPE
Convert Contracts::83
Adding Contracts::81
New Contracts::80
Renew Contracts::77
Renew Contract::58
Add Secondary Services::73


Service Rate Change::49
Resign Contract::56
Owner Change::5
Modify Contracts 2::88
Equipment Return::72
Close Contracts::79
Address Change::87
...
create table rcbill_my.sales(index idxs1(orderid), index idxs2(clientcode), index idxs3(contractcode), index idxs4(orderday), index idxs5(orderyr, ordermth, orderweek)) as 
(
		...
		, case 
			...
			when rcbill_my.GetCleanString(ORDERTYPE) = "Add Secondary Services" then "Secondary Sales" 
            
			when rcbill_my.GetCleanString(ORDERTYPE) = "Service Rate Change" then "Modification" 
			when rcbill_my.GetCleanString(ORDERTYPE) = "Resign Contract" then "Modification" 
			when rcbill_my.GetCleanString(ORDERTYPE) = "Owner Change" then "Modification" 
			when rcbill_my.GetCleanString(ORDERTYPE) = "Modify Contracts" then "Modification" 
			when rcbill_my.GetCleanString(ORDERTYPE) = "Equipment Return" then "Modification" 
			when rcbill_my.GetCleanString(ORDERTYPE) = "Close Contracts" then "Closure" 
			when rcbill_my.GetCleanString(ORDERTYPE) = "Address Change" then "Modification" 
		...
		, orderid, dayname(orderdate) as weekday, year(ORDERDATE) as orderyr, month(ORDERDATE) as ordermth, week(ORDERDATE) as orderweek, monthname(ORDERDATE) as ordermonth, date(orderdate) as orderday, orderdate, clientcode, clientclass, contract as contractcode, contracttype, service, servicetype, cost, price, num, originalcontract, ORIGINALSERVICE, ORIGINALSERVICETYPE, ORIGINALPRICE, CLEANORIGCOST, CLEANORIGPRICE, insertedon
...
-- LOAD DATA LOW_PRIORITY LOCAL INFILE 'C:\\ProgramData\\MySQL\\MySQL Server 5.7\\Uploads\\_csv\\Sales-Addon-25082019-02092019.csv'
 LOAD DATA LOW_PRIORITY LOCAL INFILE 'C:\\ProgramData\\MySQL\\MySQL Server 5.7\\Uploads\\_csv\\Sales-Addon-18112022-20112022.csv'
...
delete from rcbill_my.dailyaddonsales where date(insertedon)= '2021-12-01';
2020-10-01 09:07:37
...
-- LOAD DATA LOW_PRIORITY LOCAL INFILE 'C:\\ProgramData\\MySQL\\MySQL Server 5.7\\Uploads\\_csv\\eBarclays-PaymentsList-25082019-02092019.csv' 
 LOAD DATA LOW_PRIORITY LOCAL INFILE 'C:\\ProgramData\\MySQL\\MySQL Server 5.7\\Uploads\\_csv\\eBarclays-PaymentsList-18112022-20112022.csv' 
...
delete from rcbill_my.onlinepayments where insertedon='2021-12-01 09:51:30';

delete from rcbill_my.onlinepayments where date(insertedon)='2020-10-01';
...
-- LOAD DATA LOW_PRIORITY LOCAL INFILE 'C:\\ProgramData\\MySQL\\MySQL Server 5.7\\Uploads\\_csv\\PrepaidCardSales-25082019-02092019-P1.csv' 
 LOAD DATA LOW_PRIORITY LOCAL INFILE 'C:\\ProgramData\\MySQL\\MySQL Server 5.7\\Uploads\\_csv\\PrepaidCardSales-18112022-20112022-P1.csv' 
