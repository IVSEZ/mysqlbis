...
drop table if exists rcbill_my.rep_livetvpivot2021;

create table rcbill_my.rep_livetvpivot2021 as 
(
	...
	where view_date>='2021-01-01' and view_date<='2021-12-31'
	GROUP BY 1
	order by 1
)
;

drop table if exists rcbill_my.rep_vodpivot2021;

create table rcbill_my.rep_vodpivot2021 as 
(
...
drop table if exists rcbill_my.rep_radiopivot2021;

create table rcbill_my.rep_radiopivot2021 as 
(
	SELECT  upper(trim(resource)) as RESOURCE,
		SUM( IF( MONTH(view_date) = 1, duration_sec, 0) ) AS `JAN`,
		SUM( IF( MONTH(view_date) = 2, duration_sec, 0) ) AS `FEB`,
...
drop table if exists rcbill_my.rep_tspivot2021;

create table rcbill_my.rep_tspivot2021 as 
(
	SELECT  upper(trim(resource)) as RESOURCE,
