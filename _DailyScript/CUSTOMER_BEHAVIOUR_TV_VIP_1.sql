...
set @datee='2018-09-30';
	...
	drop table if exists rcbill_my.tempcustbehv1;
	drop table if exists rcbill_my.tempcustbehv2;


    create table rcbill_my.tempcustbehv1 as 
    (
		SELECT 
			period,
				periodday,
    ...
    create table rcbill_my.tempcustbehv2 as 
    (
    ...
    select * from rcbill_my.tempcustbehv1;
    
    select * from rcbill_my.tempcustbehv2 
    ;
    -- where standalone>1;
...
####################################################
### PIVOT
select 'VIP' as ctype, concat(standalone,"|", addon) as combo, standalone, addon, periodyear, periodmth
, count(distinct clientcode) as dcl
from 
rcbill_my.tempcustbehv2 
group by 1, 2, standalone, addon, periodyear, periodmth
order by standalone, addon, periodyear, periodmth
; 


select 'VIP' as ctype, periodyear, periodmth
, count(distinct clientcode) as dcl
from 
rcbill_my.tempcustbehv2 
