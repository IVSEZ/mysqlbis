...
drop table if exists rcbill_my.rep_livetvpivot2022;

create table rcbill_my.rep_livetvpivot2022 as 
(
	...
	where view_date>='2022-01-01' and view_date<='2022-12-31'
	GROUP BY 1
	order by 1
)
;

drop table if exists rcbill_my.rep_vodpivot2022;

create table rcbill_my.rep_vodpivot2022 as 
(
...
drop table if exists rcbill_my.rep_radiopivot2022;

create table rcbill_my.rep_radiopivot2022 as 
(
	SELECT  upper(trim(resource)) as RESOURCE,
		SUM( IF( MONTH(view_date) = 1, duration_sec, 0) ) AS `JAN`,
		SUM( IF( MONTH(view_date) = 2, duration_sec, 0) ) AS `FEB`,
...
drop table if exists rcbill_my.rep_tspivot2022;

create table rcbill_my.rep_tspivot2022 as 
(
	SELECT  upper(trim(resource)) as RESOURCE,
